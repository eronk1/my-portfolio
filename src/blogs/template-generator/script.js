    ...
    function exportSectionFiles(sec, sectionDiv) {
      ...
      // Single attachment (either one file OR one snippet)
        ...
        if (files.length === 1) {
          ...
          // In blog.json, reference only the base name.
          sec.picture = baseName;
    
          // In the ZIP: if codeAttach is true, force a .txt extension; otherwise use the original filename.
          if (sec.codeAttach) {
        ...
        // CASE 2: There's only a snippet.
          ...
          if (sec.codeAttach) {
            ...
            // Remove the extension for the JSON reference.
      ...
      // Multiple attachments (files and/or snippet)
        ...
        if (snippet) {
          if (sec.codeAttach) {
            const snippetFileName = "snippet" + (globalFileIndex++) + ".txt";
            sec.picture.push(snippetFileName); // Include extension in JSON
            picturesFolder.file(snippetFileName, snippet);
          } else {
            const snippetFileName = "snippet" + (globalFileIndex++);
        ...
        files.forEach(fileObj => {
          ...
          if (sec.codeAttach) {
            // For codeAttach true, use the stored filename (baseName + extension) in JSON.
            const ext = dotIndex !== -1 ? fileName.substring(dotIndex) : ".txt";
            const newFileName = baseName + ext;
            sec.picture.push(newFileName);
            picturesFolder.file(newFileName, fileObj);
          } else {
            // Otherwise, JSON gets just the base name.
